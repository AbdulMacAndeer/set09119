...
int main()
{
	...
	// scale it up x5
	plane.scale(glm::vec3 (7.0f, 7.0f, 7.0f));
	...
	*/
	...
	glm::vec3 u[100];
	//glm::vec3 u = glm::vec3(5.0f, 5.0f, 0.0f);
	glm::vec3 force = glm::vec3(0.0f, 1.0f, 0.0f);
	glm::vec3 r0[100];
	//glm::vec3 r0 = particle1.getTranslate()[3];

	GLfloat firstFrame[100];
	//GLfloat firstFrame = (GLfloat) glfwGetTime();

	for (int i = 0; i < 100; i++)
	{
		particles[i] = Mesh::Mesh();
		particles[i].translate(glm::vec3(0.0f, 2.5f, 0.0f));
		particles[i].scale(glm::vec3(.1f, .1f, .1f));
		particles[i].rotate((GLfloat)M_PI_2, glm::vec3(1.0f, 0.0f, 0.0f));

		// allocate shader
		particles[i].setShader(Shader("resources/shaders/core.vert", "resources/shaders/core_blue.frag"));
		//set r
		r0[i] = glm::vec3(randf(-10.0f, 10.0f), randf(0.0f, 20.0f),0.0f);
		//set u
		u[i] = glm::vec3(randf(-8.0f, 8.0f), randf(4.0f, 8.0f), 0.0f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		*/
		...
		//acc *=1.1;
		//particle1.translate(glm::vec3(0.0f, -1.5f * deltaTime * acc, 0.0f));

		// 2 - same as above using the setPos method
		//particle1.setPos(glm::vec3(0.0f, -(acc * currentFrame), 0.0f));

		// 3 - make particle oscillate above the ground plane
		//particle1.setPos(glm::vec3(0.0f, sin(-currentFrame)+1, 0.0f));
		
		// 4 - particle animation from initial velocity and acceleration = ut + 0.5a(t)^2 
		//particle1.setPos(u*currentFrame + (0.5f*a*(currentFrame*currentFrame)));

		// 5 - add collision with plane

		////position is r + ut + 0.5a(t)^2
		//particle1.setPos(r0 + u*currentFrame + (0.5f*(a*(currentFrame*currentFrame))));

		//if (particle1.getTranslate()[3][1] <= plane.getTranslate()[3][1])
		//{
		//	//set r.x to the particle, and r.y to the planes.y
		//	r0.x = particle1.getTranslate()[3][0];
		//	r0.y = plane.getTranslate()[3][1];
		//	//Calculate u(initial velocity)
		//	u += a*currentFrame;
		//	//Reverse u with force
		//	u.y = -u.y - force.y;
		//	//reset time
		...
		{
			//position is r + ut + 0.5a(t)^2
			particles[i].setPos(r0[i] + u[i]*currentFrame[i] + (0.5f*(a*(currentFrame[i]*currentFrame[i]))));

			if (particles[i].getTranslate()[3][1] <= plane.getTranslate()[3][1])
			{
				//set r.x to the particle, and r.y to the planes.y
				r0[i].x = particles[i].getTranslate()[3][0];
				r0[i].y = plane.getTranslate()[3][1];
				//Calculate u(initial velocity)
				u[i] += a*currentFrame[i];
				//Reverse u with force
				u[i].y = -u[i].y - force.y;
				//reset time
				firstFrame[i] = (GLfloat)glfwGetTime();
